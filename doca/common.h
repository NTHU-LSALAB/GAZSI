/*
 * GPU Packet Processing - Common Header
 * SPDX-License-Identifier: BSD-3-Clause
 */

#ifndef GPU_PKT_COMMON_H
#define GPU_PKT_COMMON_H
...
struct app_gpu_cfg {
	...
	char engine_path[512];				/* TensorRT engine file path */
};
...
struct rxq_tcp_queues {
	...
	/* Root pipe for traffic routing */
	struct doca_flow_pipe *root_pipe;		    /* Root pipe for all traffic */
	struct doca_flow_pipe_entry *root_tcp_gpu_entry;    /* Root entry for TCP to GPU */
	struct doca_flow_pipe_entry *root_tcp_cpu_entry[3]; /* Root entries for TCP SYN/RST/FIN to CPU */
	...
	/* Semaphore for CPU → GPU inference result notification */
	struct doca_gpu_semaphore *sem_inference_cpu;      /* CPU handle */
	struct doca_gpu_semaphore_gpu *sem_inference_gpu;  /* GPU handle */

	/* Semaphore for GPU → CPU request notification */
	struct doca_gpu_semaphore *sem_request_cpu;      /* CPU handle */
	struct doca_gpu_semaphore_gpu *sem_request_gpu;  /* GPU handle */
};
...
struct info_http {
	...
	char param_value[64];			    /* Extracted parameter value */
};

/* Defined in tcp/session.h */
...
/*
 * Create DOCA Flow root pipeline to route TCP traffic.
 *
 * @tcp_queues [in]: Application TCP queues
 * @port [in]: DOCA Flow port associated to the DOCA device
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
...
/*
 * Set UVM buffer for inference data storage
 *
 * @stream [in]: CUDA stream
 * @buf [in]: UVM buffer pointer
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
