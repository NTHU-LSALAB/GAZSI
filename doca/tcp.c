/*
 * GPU Packet Processing - TCP Queue Management
			       ...
			       doca_eth_txq_gpu_event_error_send_packet_cb_t event_error_send_packet_cb)
{
	...
	for (uint32_t idx = 0; idx < queue_num; idx++) {
		...
		if (http_server) {
			...
			/* Create inference result notification semaphore
			 * Only create once (idx == 0) since ring buffer is global
			 */
			if (idx == 0) {
				result = doca_gpu_semaphore_create(tcp_queues->gpu_dev, &(tcp_queues->sem_inference_cpu));
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_create for inference: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				/* CPU_GPU: CPU can write (set READY), GPU can read (poll status) */
				result = doca_gpu_semaphore_set_memory_type(tcp_queues->sem_inference_cpu,
									    DOCA_GPU_MEM_TYPE_CPU_GPU);
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_set_memory_type for inference: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				/* 128 items, one for each ring buffer slot */
				result = doca_gpu_semaphore_set_items_num(tcp_queues->sem_inference_cpu,
									  INFERENCE_RING_SIZE);
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_set_items_num for inference: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				result = doca_gpu_semaphore_start(tcp_queues->sem_inference_cpu);
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_start for inference: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				result = doca_gpu_semaphore_get_gpu_handle(tcp_queues->sem_inference_cpu,
									   &(tcp_queues->sem_inference_gpu));
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_get_gpu_handle for inference: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				DOCA_LOG_INFO("Created sem_inference: %d items for CPU to GPU notification",
					      INFERENCE_RING_SIZE);

				/* Create GPU to CPU request notification semaphore */
				result = doca_gpu_semaphore_create(tcp_queues->gpu_dev, &(tcp_queues->sem_request_cpu));
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_create for request: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				/* CPU_GPU: GPU can write (set READY), CPU can read (poll status) */
				result = doca_gpu_semaphore_set_memory_type(tcp_queues->sem_request_cpu,
									    DOCA_GPU_MEM_TYPE_CPU_GPU);
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_set_memory_type for request: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}
				...
				/* 128 items, one for each ring buffer slot */
				result = doca_gpu_semaphore_set_items_num(tcp_queues->sem_request_cpu,
									  INFERENCE_RING_SIZE);
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_set_items_num for request: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				result = doca_gpu_semaphore_start(tcp_queues->sem_request_cpu);
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_start for request: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				result = doca_gpu_semaphore_get_gpu_handle(tcp_queues->sem_request_cpu,
									   &(tcp_queues->sem_request_gpu));
				if (result != DOCA_SUCCESS) {
					DOCA_LOG_ERR("Failed doca_gpu_semaphore_get_gpu_handle for request: %s",
						     doca_error_get_descr(result));
					destroy_tcp_queues(tcp_queues, http_server, http_queues);
					return DOCA_ERROR_BAD_STATE;
				}

				DOCA_LOG_INFO("Created sem_request: %d items for GPU to CPU notification",
					      INFERENCE_RING_SIZE);
			}
	...
	if (http_server) {
		...
		result = prepare_tx_buf(&http_queues->buf_page_index, HTTP_GET_INFERENCE);
		if (result != DOCA_SUCCESS) {
				...
				struct txq_http_queues *http_queues)
{
	...
	if (http_server) {
		/* Destroy inference semaphore */
		if (tcp_queues->sem_inference_cpu) {
			result = doca_gpu_semaphore_stop(tcp_queues->sem_inference_cpu);
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Failed doca_gpu_semaphore_stop for inference: %s",
					     doca_error_get_descr(result));
				return DOCA_ERROR_BAD_STATE;
			}

			result = doca_gpu_semaphore_destroy(tcp_queues->sem_inference_cpu);
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Failed doca_gpu_semaphore_destroy for inference: %s",
					     doca_error_get_descr(result));
				return DOCA_ERROR_BAD_STATE;
			}
			tcp_queues->sem_inference_cpu = NULL;
			tcp_queues->sem_inference_gpu = NULL;
		}

		/* Destroy request notification semaphore */
		if (tcp_queues->sem_request_cpu) {
			result = doca_gpu_semaphore_stop(tcp_queues->sem_request_cpu);
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Failed doca_gpu_semaphore_stop for request: %s",
					     doca_error_get_descr(result));
				return DOCA_ERROR_BAD_STATE;
			}

			result = doca_gpu_semaphore_destroy(tcp_queues->sem_request_cpu);
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Failed doca_gpu_semaphore_destroy for request: %s",
					     doca_error_get_descr(result));
				return DOCA_ERROR_BAD_STATE;
			}
			tcp_queues->sem_request_cpu = NULL;
			tcp_queues->sem_request_gpu = NULL;
		}
