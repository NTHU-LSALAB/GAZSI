/*
 * GPU Packet Processing - Flow Rules
...
/*
 * =============================================================================
 * TCP CPU Fallback Pipe
 * =============================================================================
 *
 * Purpose: Handle TCP control packets (SYN/RST/FIN) that require CPU processing
 *
 * Flow:
 *   NIC → [Root Pipe] → [TCP CPU Pipe] → DPDK RSS queues (CPU processing)
 *
 * Match:  L3=IPv4, L4=TCP (matched by Root Pipe with SYN/RST/FIN flags)
 * Action: RSS distribute to DPDK queues for CPU TCP stack handling
 *
 * This pipe is used for:
 *   - TCP connection establishment (SYN/SYN-ACK)
 *   - TCP connection termination (FIN/RST)
 *   - Packets from unknown flows not yet offloaded to GPU
 *
 * The Root Pipe forwards SYN/RST/FIN packets here with high priority,
 * while established data flows go to the GPU pipe with lower priority.
 */
 ...
 * =============================================================================
 *
 * Purpose: Route established TCP data flows to GPU queues for accelerated processing
 *
 * Flow:
 *   NIC → [Root Pipe] → [TCP GPU Pipe] → GPU RX queues (RSS distribution)
 *
 * Match:  L3=IPv4, L4=TCP
 *         Optional: 5-tuple (src_ip, dst_ip, src_port, dst_port) for connection-based
 * Action: RSS distribute to GPU queues based on 5-tuple hash
 *
 * This pipe handles:
 *   - Established TCP connections (DATA packets)
 *   - HTTP request/response payload processing on GPU
 *
 * Connection-based mode:
 *   When connection_based_flows=true, flow entries are dynamically added
 *   per TCP session by enable_tcp_gpu_offload() after CPU handles SYN handshake.
 *   This provides per-connection flow steering.
 *
 * Non-connection-based mode:
 *   When connection_based_flows=false, a single catch-all entry forwards
 *   all TCP traffic to GPU (simpler but less precise).
 */
...
/*
 * =============================================================================
 * Root Pipe (Traffic Director)
 * =============================================================================
 *
 * Purpose: Entry point for all incoming packets - routes to protocol-specific pipes
 *
 * Flow:
 *   NIC → [Root Pipe] ─┬─→ [TCP GPU Pipe]  → GPU queues (DATA, low priority)
 *                      └─→ [TCP CPU Pipe]  → CPU queues (SYN/RST/FIN, high priority)
 *
 * Type: DOCA_FLOW_PIPE_CONTROL (programmable entries, not pattern-based)
 * Root: true (first pipe to receive all traffic from NIC)
 *
 * Routing Rules:
 *   - TCP SYN/RST/FIN      → TCP CPU Pipe (priority=1, high)
 *   - TCP DATA             → TCP GPU Pipe (priority=3, low)
 *
 * The priority system ensures TCP control packets (SYN/RST/FIN) are handled
 * by CPU for connection management, while data packets go to GPU for
 * high-throughput processing.
 */
doca_error_t create_root_pipe(struct rxq_tcp_queues *tcp_queues,
			      struct doca_flow_port *port)
{
	...
	if (tcp_queues->rxq_pipe_cpu) {
		...
		for (int i = 0; i < 3; i++) {
			...
			result = doca_flow_pipe_control_add_entry(0,
								  ...
								  &tcp_fwd_cpu,
								  NULL,
								  &tcp_queues->root_tcp_cpu_entry[i]);
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Root pipe TCP CPU entry creation failed with: %s",
					     doca_error_get_descr(result));
				return result;
			}
		}
