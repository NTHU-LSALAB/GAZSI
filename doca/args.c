/*
 * GPU Packet Processing - Argument Parser
...
/*
 * Get TensorRT engine path input.
 *
 * @param [in]: Command line parameter
 * @config [in]: Application configuration structure
 * @return: DOCA_SUCCESS on success and DOCA_ERROR_INVALID_VALUE otherwise
 */
static doca_error_t engine_path_callback(void *param, void *config)
{
	struct app_gpu_cfg *app_cfg = (struct app_gpu_cfg *)config;
	char *engine_path = (char *)param;
	int len;

	len = strnlen(engine_path, 512);
	if (len >= 512) {
		DOCA_LOG_ERR("Engine path too long. Max 511 characters");
		return DOCA_ERROR_INVALID_VALUE;
	}

	strlcpy(app_cfg->engine_path, engine_path, 512);
...
{
	doca_error_t result;
	struct doca_argp_param *gpu_param, *nic_param, *queue_param, *server_param, *engine_param;
	...
	result = doca_argp_param_create(&engine_param);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to create ARGP param: %s", doca_error_get_descr(result));
		return result;
	}

	doca_argp_param_set_short_name(engine_param, "e");
	doca_argp_param_set_long_name(engine_param, "engine");
	doca_argp_param_set_arguments(engine_param, "<TensorRT engine path>");
	doca_argp_param_set_description(engine_param, "Path to TensorRT engine file for inference");
	doca_argp_param_set_callback(engine_param, engine_path_callback);
	doca_argp_param_set_type(engine_param, DOCA_ARGP_TYPE_STRING);
	result = doca_argp_register_param(engine_param);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to register ARGP param: %s", doca_error_get_descr(result));
		return result;
	}
